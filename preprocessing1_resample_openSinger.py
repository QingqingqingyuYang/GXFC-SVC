...
def resample_one(filename):
    singer="OpenSinger-" + filename.split('/')[-3].split('R')[-2] + filename.split('/')[-2].split('_')[-2]
    songname=filename.split('/')[-2].split('_')[-1]
    wavnum=filename.split('/')[-1].split('_')[-1] 
    ...
    output_path=output_path + wavnum 
    try:
...
def mkdir_func(input_path):
    singer="OpenSinger-" + input_path.split('/')[-3].split('R')[-2] + input_path.split('/')[-2].split('_')[-2] 
    out_dir = 'dataset/'+singer
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

def resample_parallel(num_process):
    input_paths = glob('dataset_raw/OpenSinger/*/*/*.wav')  
