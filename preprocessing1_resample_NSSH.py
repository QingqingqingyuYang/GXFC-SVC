...
def resample_one(filename):
    singer="NSSH-" + filename.split('/')[-4]
    songname=filename.split('/')[-3] + "-"
...
def mkdir_func(input_path):
    singer="NSSH-" + input_path.split('/')[-4]
    out_dir = 'dataset/'+singer
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

def resample_parallel(num_process):
    input_paths = glob('dataset_raw/Sing_Voce_Conversion_Challenge_NSSHDATASET/*/*/*/*.wav')
