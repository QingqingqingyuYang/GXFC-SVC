...
def process(filename,wavformat,size):
    songname = "NUS-" + filename.split('/')[-2] + filename.split('/')[-1].strip('.'+wavformat)
    singer = "NUS-" + filename.split('/')[-3]
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s","--size",  type=int, default=7000, help="the length of audio slices")
    args = parser.parse_args()
    wavformat = args.wavformat
    size = args.size
    files=glob('./dataset_slice/nus-smc-corpus_48/*/*/*.'+wavformat)
    print("input_paths",len(files)) 
    for file in files:
