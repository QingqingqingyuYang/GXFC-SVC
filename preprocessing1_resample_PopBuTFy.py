...
def resample_one(filename):
    singer="PopBuTFy-" + filename.split('/')[-2].split('#')[-3]
    songname=filename.split('/')[-2].split('#')[-1]
...
def mkdir_func(input_path):
    singer="PopBuTFy-" + input_path.split('/')[-2].split('#')[-3] 
    out_dir = 'dataset/'+singer
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

def resample_parallel(num_process):
    input_paths = glob('dataset_raw/PopBuTFy/*/*.wav')  
